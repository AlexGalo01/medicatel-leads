...
class DirectoryEntriesRepository:
    ...
    async def bulk_create(self, entries: list[ExaRawEntry]) -> None:
    ...
    async def list_by_job(
        ...
        page: int = 1,
        page_size: int = 20,
    ) -> tuple[list[ExaRawEntry], int]:
        base = select(ExaRawEntry).where(ExaRawEntry.job_id == job_id)
        ...
        page_query = (
            base.order_by(ExaRawEntry.created_at.asc()).offset(offset).limit(page_size)
        )
